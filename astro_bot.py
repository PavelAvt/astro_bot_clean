...
def generate_natal_analysis(birth_date, birth_time, city):
    ...
    prompt = (
        ...
        f"Составь подробный настоящий астрологический прогноз в стиле личной консультации основываясь на полученых данных."
    )
    try:
...
def zodiac_handler(msg):
    ...
    if chat_id in user_data and user_data[chat_id].get("date") == today:
        bot.send_message(chat_id,
            ...
            parse_mode="HTML")
        return
    prompt = f"Составь настоящий астрологический гороскоп на сегодня для знака {msg.text} ({today}) в 3–4 предложениях."
    try:
        response = openai_client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": prompt}]
...
def send_daily_horoscopes():
    ...
    for user in users:
        if not user["sign"]:
            continue
        prompt = f"Составь настоящий астрологический гороскоп на сегодня для {user['sign']} на сегодня."
        try:
