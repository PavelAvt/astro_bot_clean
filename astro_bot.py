...
def generate_natal_analysis(birth_date, birth_time, city):
    prompt = (
        f"Ты — профессиональный астролог. Проведи творческий, глубокий и подробный анализ личности "
        f"на основе даты рождения {birth_date}, времени {birth_time} и города рождения {city}. "
        f"Предположи положения Солнца, Луны, Меркурия, Марса, их влияние, характер человека, "
        f"его эмоциональность, темперамент, стиль общения, таланты, вызовы и советы для развития. "
        f"Пиши как для личной консультации клиента. Стиль — тёплый, поэтичный, 5–7 абзацев."
    )
    try:
        response = openai_client.chat.completions.create(
            model="gpt-3.5-turbo",
